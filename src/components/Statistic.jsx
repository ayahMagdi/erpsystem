  ...
  return (
    ...
    <div className={`rounded-[25px] p-4 ${bg} bg-hero-pattern bg-no-repeat bg-contain bg-center`} onClick={() => handleBranch(branch)}>
        <div className='p-2 text-white'>
            <h3 className='text-md md:text-lg xl:text-2xl font-semibold '><CountUp start={0} end={endCount} duration={2.5} />{prod ? <span className=' text-xl'> - {prod}</span> : ''}</h3>
            <h4 className='text-xs md:text-base font-semibold'>{title}</h4>
